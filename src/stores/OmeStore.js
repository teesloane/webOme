...
export class OmeStore {
  ...
  @computed get bpmTime() {
    return 60 / this.tempo * 1000 / this.grid;
  }
  ...
  @action selectGrid = newGrid => {
    this.grid = newGrid.value;
  };

  @action selectMidiDevice = newDevice => {
    this.selectedMidiOut = newDevice.value;
  };

  @action selectKey = newKey => {
  ...
  @action toggleOmeBtn = (rowId, noteId) => {
    this.midiNotes[rowId][noteId].noteOn = !this.midiNotes[rowId][noteId]
      .noteOn;
  };
  ...
  playNote = () => {
    ...
    this.onNotes.forEach(note => {
      let noteToPlay =
        this.midiNotes[this.currentRow][note].midiNote + this.octave * 12;
  ...
  playOme() {
    ...
    this.playNote();
    setTimeout(() => {
      requestAnimationFrame(() => this.playOme());
    }, this.bpmTime);
  }
  ...
  createNotes = scale => {
    for (let i = 0; i < 16; i++) {
      // hard code 16, numSteps is for ui only?
      let newOmeRow = {};
      newOmeRow[`row_${i}`] = {};
  ...
  createSelectorOptions(arr, keyLabel) {
    return arr.map(item => {
      return { label: item[keyLabel], value: item };
    });
  }
  ...
  getMidiAccess = () => {
    if (navigator.requestMIDIAccess) {
      navigator.requestMIDIAccess().then(this.midiSuccess, () => {
  ...
  midiSuccess = midiAccess => {
    ...
    for (let midiOutput of midiAccess.outputs) {
      this.midiOutputs.push(midiOutput[1]);
    }
    for (let midiInput of midiAccess.inputs) {
      this.midiInputs.push(midiInput[1]);
    }
